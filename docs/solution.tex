...
\begin{center}
    \small Repositorio: \href{https://github.com/DanielBarillasM/Proyecto-03_Eleccion-de-tecnologias-de-base-de-datos_BD2_seccion-20_Grupo-01}{GitHub — Proyecto 3}
\end{center}
...
Las figuras 1 y 2 muestran dos rompecabezas físicos
etiquetados siguiendo esta convención.

\begin{figure}[h]
    \centering
    \includegraphics[width=0.78\textwidth]{figs/ples.jpeg}
    \caption{Rompecabezas \emph{Plesiosaurs}: 10 piezas numeradas y conectores
             etiquetados con letras (A, B, C\ldots) junto a cada tab o hueco.}
    \label{fig:ples}
\end{figure}

\begin{figure}[h]
    \centering
    \includegraphics[width=0.78\textwidth]{figs/exc.jpeg}
    \caption{Rompecabezas \emph{Excavator}: 10 piezas numeradas y conectores
             etiquetados con letras (A, B, C, D) junto a cada conector.}
    \label{fig:exc}
\end{figure}
...
\begin{description}[leftmargin=3cm, style=sameline]
    ...
    \item[\texttt{:Piece}]     \texttt{id} (global), \texttt{local\_id}, \texttt{label}, \texttt{puzzle\_id}, \texttt{available} (bool)
...
La disponibilidad de cada pieza se almacena directamente en la base de datos
como una propiedad booleana \texttt{available} en el nodo \texttt{:Piece}.
Esto significa que el estado de faltante es persistente entre sesiones y
compartido por todos los usuarios del sistema, en lugar de ser un parámetro
transitorio pasado en tiempo de ejecución.

\begin{lstlisting}[language=SQL, caption={Actualizar disponibilidad de una pieza en Neo4j}]
MATCH (pc:Piece {id: $global_id})
SET pc.available = false;   -- marcar como faltante
SET pc.available = true;    -- restaurar
\end{lstlisting}

Al iniciar el BFS, el sistema consulta el grafo cargado en memoria y deriva
automáticamente el conjunto de piezas faltantes $M$ a partir de los nodos
con \texttt{available = false}:

\[
    M = \{\, p \in P \mid p.\texttt{available} = \texttt{false} \,\}
\]

Desde la interfaz, el usuario puede marcar o restaurar cualquier pieza con un
botón por pieza antes de resolver. El cambio se persiste en Neo4j
inmediatamente. Cuando durante el BFS se encuentra una pieza $v \in M$:
...
Al restaurar una pieza en la interfaz, el cambio se escribe a Neo4j y la
siguiente resolución ya la incluye automáticamente.
